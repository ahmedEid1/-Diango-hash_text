...
def home(request):
    if request.method == "POST":
        ...
        if filled_form.is_valid():
            ...
            text_hash = hashlib.sha3_256(text.encode('utf-8')).hexdigest()
            try:
                Hash.objects.get(hash=text_hash)
            except Hash.DoesNotExist:
                hash = Hash(text=text, hash=text_hash)
                hash.save()
...
def quichash(request):
    text = request.GET['text']
    text = request.GET['text']
    return JsonResponse({'hash': hashlib.sha3_256(text.encode('utf-8')).hexdigest()})
